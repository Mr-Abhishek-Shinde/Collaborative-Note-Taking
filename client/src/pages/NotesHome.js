...
//   <li key={note._id} onClick={() => navigate(`/notes/note/${note._id}`, { state: { isShared: true } })}>
//     <i className="fa-regular fa-note-sticky"></i> {note.title} notes
//   </li>
// ))}
...
import styles from "../styles/NotesHome.module.css";
import notesHome from "../image/notesHome.png";
import SubNavbar from "../components/SubNavbar"; // Import SubNavbar component
...
const NotesHome = () => {
  ...
  const [notesList, setNotesList] = useState([]);
  const [sharedNotesList, setSharedNotesList] = useState([]);
  const [isSharedNote, setIsSharedNote] = useState(false); // New state variable
  ...
  const saveClickedSharedNote = (noteId) => {
    console.log("Clicked shared note ID:", noteId);
    setIsSharedNote(true); // Set isSharedNote to true
    navigate(`/notes/note/${noteId}`); // Navigate to the note without state
  };
